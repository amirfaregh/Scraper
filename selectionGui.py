    ...
    if len(price_entry.get()) == 0:
        ...
        for page_num in range(1, 2):
            if len(link_entry.get()) == 0:
                web_address = ('http://www.trendyol.com' +
                               select.get()+sorting.get())
            else:
...
for text, sort in SORTS:
    Radiobutton(root, text=text, variable=sorting,value=sort).pack(side=LEFT, anchor=N)
